...
export function handleTouchMove(event, setMousePosition) {
  const mouseX = (event.touches[0].clientX / window.innerWidth) * 2 - 1
  const mouseY = -(event.touches[0].clientY / window.innerHeight) * 2 + 1
  setMousePosition(mouseX, mouseY)
}

export function handleTouchEnd(setMousePosition) {
  setTimeout(() => {
...
export function handleHeadRotation(headBone, mouseX, mouseY, interpolationX, interpolationY, lerp) {
  if (!headBone) return
